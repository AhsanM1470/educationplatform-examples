...
5. Last but not least, add the rule Feature as defined below:
   ```Feature: (many?='many')? name=ID ':' type=[Type];```. The keyword **many** shall be used to model a multi-valued feature in this DSL. The assignment operator (?=) implies that the feature **many** is of type _boolean_. You are already familiar with the other syntax elements in this parser rule. <!-- {pointed: panel-xtext} -->
